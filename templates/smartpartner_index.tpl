...
<{if count($partners) > 0}><{foreach item=category from=$partners}><{if $category.title}>
    ...
    <table>
        <{foreach item=partner from=$category.partners}> <{if $partner.display_type == 'part' || $partner.display_type == 'full'}>
            <tr valign="top">
                ...
                <td class='<{cycle values="even, odd"}>' style="padding: 5px;" align='left' valign='top'>
                    ...
                    <{if $partner.readmore}> <{if $partner.display_type == 'full'}>
                        <div class="smartpartner_readmore">
                            <a href="<{$smartPartner_url}>partner.php?id=<{$partner.id}>&cid=<{$category.categoryid}>"><{$lang_readmore}></a>
                        </div>
                    <{else}>
                        <div><b><{$partview_msg}></b></div>
                    <{/if}> <{/if}>
                </td>
            </tr>
        <{/if}> <{/foreach}>
        <tr>
            <td colspan="2">
                <!--removed by ampersand <{if $category.title && $on_index_page}>
                       <div class="smartpartner_category_viewall">
                           <a href="<{$category.link_view}>"><{$view_deteils_cat}></a>
                       </div>
                   <{/if}>-->
            </td>
        </tr>
...
<table cellspacing='0' border='0' width='98%'>
    <tr>
        <td width='50%' align='left'>
            <{if $displayjoin}>
                <a href="<{$smartPartner_url}>join.php"><{$lang_join}></a>
            <{/if}>
        </td>
        <td width='50%' align='right'><{$pagenav}></td>
    </tr>
</table>
